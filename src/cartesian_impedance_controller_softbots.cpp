...
bool CartesianImpedanceControllerSoftbots::init(hardware_interface::RobotHW* robot_hw,
  ...
  cartesian_stiffness_.topLeftCorner(3, 3) << 400*Eigen::Matrix3d::Identity();
  cartesian_stiffness_.bottomRightCorner(3, 3) << 40*Eigen::Matrix3d::Identity();
  cartesian_stiffness_target_ = cartesian_stiffness_;
  // Damping ratio = 1

  cartesian_damping_.topLeftCorner(3, 3) = 2.0 * sqrt (400)*Eigen::Matrix3d::Identity();
  cartesian_damping_.bottomRightCorner(3, 3) = 2.0 * sqrt(40)*Eigen::Matrix3d::Identity();
